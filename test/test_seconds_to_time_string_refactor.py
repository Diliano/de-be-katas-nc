from src.seconds_to_time_string_refactor import seconds_to_time_string
import pytest

def test_handles_input_that_evaluates_only_to_seconds():
    assert seconds_to_time_string(0) == "0 seconds"
...
def test_raises_value_error_if_input_is_less_than_0():
    with pytest.raises(ValueError) as excinfo:
        seconds_to_time_string(-10)
    assert str(excinfo.value) == "Input must be 0 or greater"

def test_raises_value_error_if_input_is_wrong_type():
    with pytest.raises(TypeError) as excinfo:
        seconds_to_time_string("ten")
